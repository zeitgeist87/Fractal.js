...
var Fractal = (function (Fractal, Color){
	...
	// constructor
	function HTree(canvas, maxorder, pos, startcolor, stopcolor, alpha) {
		if (!(this instanceof HTree))
			return new HTree(canvas, maxorder, pos, startcolor, stopcolor,
					alpha);
		...
		this.diagonal_length = Math.sqrt(this.width * this.width + this.height
				* this.height);
	...
	HTree.prototype = {
		...
		draw : function(curr_order, width, height) {
			var ctx = this.ctx;
